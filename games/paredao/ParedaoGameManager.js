...
const { PAREDAO_DEFAULTS } = require('./constants');
...
class GameManager {
  ...
  getTurnSettings(groupId) {
    return this.turnSettingsByGroup.get(groupId) || {
      turnDurationMinutes: this.TURN_DURATION,
      updateIntervalMinutes: this.UPDATE_INTERVAL
    };
  }

  configureTurnSettings(groupId, settings = {}) {
    const merged = {
      turnDurationMinutes: settings.turnDurationMinutes || this.TURN_DURATION,
      updateIntervalMinutes: settings.updateIntervalMinutes || this.UPDATE_INTERVAL
    };
    this.turnSettingsByGroup.set(groupId, merged);
    return merged;
  }
  ...
  async startTurn(gameId, groupId, player) {
    ...
    await this.sendTurnInstructions(player, groupId);
    await this.notifyAllPlayers(gameId, groupId, player, 'turn_started');
  ...
  async announcePlayerTurn(groupId, player) {
    try {
      ...
      const settings = this.getTurnSettings(groupId);
      ...
      announcement += `📝 *COMO FUNCIONA:*\n` +
        ...
        `⏰ *Duração:* ${settings.turnDurationMinutes} minutos\n` +
        `🔄 *Atualizações:* A cada ${settings.updateIntervalMinutes} minutos\n\n` +
        `🎯 *VAI COM TUDO!*`;
  ...
  async sendTurnInstructions(player, groupId) {
    try {
      const settings = this.getTurnSettings(groupId);
      let dmChat = null;
      if (player.dm_id) {
      ...
      const instructions = `🎤 *SEU TURNO NO PAREDÃO!* 🎤\n\n` +
        ...
        `⏰ *Duração:* ${settings.turnDurationMinutes} minutos\n` +
        `🔄 *Atualizações:* ${settings.updateIntervalMinutes} em ${settings.updateIntervalMinutes} min\n\n` +
        `🔥 *BOA SORTE!*`;
  ...
  async notifyAllPlayers(gameId, groupId, currentPlayer, type) {
    try {
      const players = await db.getGamePlayers(gameId);
        ...
        try {
          ...
          if (type === 'turn_started') {
            await dmChat.sendMessage(
              ...
              `⏰ O turno dura ${this.getTurnSettings(groupId).turnDurationMinutes} minutos\n\n` +
  ...
  startTurnTimer(gameId, groupId, player) {
    ...
    const interval = setInterval(async () => {
      ...
      if (minutes % settings.updateIntervalMinutes === 0) {
        await this.sendTurnUpdate(groupId, player, minutes);
      }
      
      if (minutes >= settings.turnDurationMinutes) {
  ...
  async sendTurnUpdate(groupId, player, minutesElapsed) {
    try {
      ...
      const update = `🔄 *ATUALIZAÇÃO* (${minutesElapsed}/${settings.turnDurationMinutes}min)\n\n` +
        `🎤 *${player.name} no Paredão*\n` +
        `📨 Perguntas: ${turnData.questionsReceived}\n` +
        `✅ Respondidas: ${turnData.questionsAnswered}\n` +
        `⏰ Restante: ${settings.turnDurationMinutes - minutesElapsed} min\n\n` +
  ...
  async endTurn(gameId, groupId) {
    ...
    if (player) {
      ...
      await this.notifyAllPlayers(gameId, groupId, player, 'turn_ended');
    }
  ...
  async finishGame(gameId, groupId) {
    ...
    this.stopTurnTimer(groupId);
    this.turnSettingsByGroup.delete(groupId);
