...
` +
    ...
    if (command === '!encerrarpartilhas') {
      if (!isAdmin && !isSupremo) {
        await msg.reply('❌ Apenas administradores podem encerrar partilhas.');
        return true;
      }
  ...
  async handle({ msg, chat, senderId, command, args }) {
    ...
    if (command === '!comandos' || command === '!help') {
      await msg.reply(
        ...
        `🕵️ *IMPOSTOR*\n` +
        `!iniciarimpostor, !partilhas N, !encerrarinscricoes, !encerrarpartilhas\n` +
    ...
    if (command === '!comojogar') {
      await msg.reply(
        ...
        `5) Em círculo, cada jogador fala 1 dica por vez com !fala ...\n` +
